package org.seekloud.yubel.util

import scala.scalajs.js.Date

/**
  * Created by dry on 2019/1/7.
  **/
object TimeTool {
  ...
  def getTimeHeader(timestamp: Long) = {
    val c = new Date(timestamp)
    val week = c.getDay() match {
      case 0 =>
        "星期日"
      case 1 =>
        "星期一"
    ...
    DateFormatter(new Date(timestamp), "yyyy-MM-dd") + "  " + week
  }
